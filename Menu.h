...
typedef enum menu { START_MENU, LEVEL_MENU, SCORE_MENU } menu;
...
class Menu_item {
 public:
  Menu_item(std::string text, menu_actions menu_action)
      : text(text), menu_action(menu_action) {}
  virtual ~Menu_item() {}
...
class Level_Menu_item : public Menu_item {
 public:
  Level_Menu_item(std::string text, menu_actions menu_action,
                  std::filesystem::path path_level)
      : Menu_item(text, menu_action), path_level(path_level) {}

...
void Engine::UpdateEntity(Entity *entity, std::vector<Component::Tag> &tags,
                          bool remove) {
  entityStream->EntityUpdated(entity, tags, remove);
}

std::vector<Entity *>::iterator Engine::RemoveEntity(Entity *entity) {
  std::vector<Entity *>::iterator re =
      std::find(vec_entitys.begin(), vec_entitys.end(), entity);
...
std::vector<System *>::iterator Engine::RemoveSystem(System *system) {
  std::vector<System *>::iterator re =
      std::find(vec_systems.begin(), vec_systems.end(), system);
...
std::vector<Entity *> &Engine::GetEntities() { return vec_entitys; }
